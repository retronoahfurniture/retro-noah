...
export default function AdminProductsPage() {
  ...
  async function quickToggle(product: Product, field: 'is_new' | 'featured') {
    ...
    try {
      ...
      const saved = await res.json()
      setProducts((prev) => prev.map((p) => (p.id === saved.id ? saved : p)))
      showToast(field === 'is_new' ? (updated.is_new ? 'Marked as New' : 'New badge removed') : (updated.featured ? 'Marked as Featured' : 'Removed from Featured'))
    } catch (e) {
      showToast(String(e), false)
    }
  }
    ...
    <div>
      ...
      {!loading && products.length === 0 && (
        <div className="bg-amber-50 border border-amber-200 p-5 mb-6 text-sm">
          ...
          <p className="text-amber-700 mb-3">Run the SQL in <code className="bg-amber-100 px-1">migration.sql</code> in your Supabase SQL Editor, then seed the data.</p>
          <button
            onClick={async () => {
